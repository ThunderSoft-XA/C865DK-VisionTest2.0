...
public class GestureClassificationTask {
    ...
    class GestureClassificationTaskThread extends Thread {
        ...
        public void run() {
            ...
            while (istarting) {
                try {
                    ...
                    //saveToBitmapfile(bitmap);
                    //FileInputStream fis = new FileInputStream(SAVE_PATH +  "test.JPEG");
        ...
        private void postObjectDetectResult(String label) {
            if (gestureClassificationTask != null) {
                gestureClassificationTask.onDraw(label);
            }
        }
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (i == size - 1) {
                    if (label.getValue() > THRESHOLD) {
                        postObjectDetectResult(label.getKey());
                    } else {
                        postNoResult();
                    }
                }
